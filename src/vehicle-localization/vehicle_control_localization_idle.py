...
LOCALIZATION_OUTPUT_FOLDER = os.path.dirname(os.path.realpath(__file__)) +\
                           '/localization_output/'
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        trajectory_fig = lp_traj.plot_new_dynamic_2d_figure(
                title='Trayectoria real y estimación de posición',
        ...
        trajectory_fig.add_graph("start_pos", window_size=1, 
                                 ...
                                 markertext="Inicio", marker_text_offset=5)
        # Add end position marker
        trajectory_fig.add_graph("end_pos", window_size=1, 
                                 x0=[waypoints_np[-1, 0]], 
                                 y0=[waypoints_np[-1, 1]],
                                 marker="D", color='r', 
                                 markertext="Fin", marker_text_offset=15)
        # Add car marker
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            ### STORING LOCALIZATION HISTORY
            x_accel_history.append(x_accel) 
            y_accel_history.append(y_accel)
